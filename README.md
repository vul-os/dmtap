...
**Conformance coverage (honest status).** The [`conformance/`](conformance/) catalog has **350
...
vectors** in `vectors/pub_vectors.json`) plus 6 self-contained canonical-CBOR reject cases; **269
...
deployment review rather than by a runner (56 + 6 + 269 + 19 = 350). Of the (non-self-contained)
...
is **62/350 today**, growing as each subsystem gains a fixed-input KAT.
