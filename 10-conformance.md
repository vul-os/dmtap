...
- **`conformance/SUITE.md`** — the normative test-case catalog: 350 numbered cases
...
- **`conformance/suite.json`** — the machine-readable mirror of those cases, so a runner in **any
  language** can drive them. It mirrors all 350 (SUITE.md and suite.json are in sync — the wave-2
...
62 + 19 + 269 = 350. An implementation conforms at a level iff it passes every `MUST` case of that
...
cases that **pass** (62 of 350 are byte-runnable today, and no implementation has yet been run
