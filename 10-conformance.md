...
- **`conformance/SUITE.md`** — the normative test-case catalog: 308 numbered cases
...
- **`conformance/suite.json`** — the machine-readable mirror of those cases, so a runner in **any
  language** can drive them. It mirrors all 308 (SUITE.md and suite.json are in sync — the wave-2
...
`conformance/SUITE.md`, each naming the review that settles it. The remaining 239 carry an exact construction recipe and expected §21 error for the
...
checklists — see `conformance/README.md`). The partition is exact: 52 + 17 + 239 = 308. An
