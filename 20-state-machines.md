...
| `ADDR_OK` | `check_duplicate` = duplicate_of_acked | `ACKED` | Dedup: `id` was **previously acked** (§2.6, §19.3.2). Ack immediately, do not reprocess. **The test is "previously acked", NOT "already held"** — see the next row. **[fill]** §2.7's ordered list does not state whether the duplicate check precedes or follows `verify_sender_sig`; this machine checks duplicate first, consistent with the "cheapest-and-anonymous-first" ordering principle stated in §2.7's own preamble. That ordering is safe **only** because the shortcut is scoped to already-acked ids: an ack the recipient has already emitted for this `id` reveals nothing further. |
| `ADDR_OK` | `check_duplicate` = duplicate_of_deferred | `DEFERRED` | The `id` is **held** — in the requests area — but was **never acked** (§2.7a). Re-delivery MUST NOT be acked and MUST NOT shortcut the §9 gate: no ack, no reprocessing, retention timer unchanged, `DEFERRED` remains terminal for this `id`. This is the ordinary path, not an attack: a cold sender's own §20.1 retry resends the identical immutable `Envelope` (same content-address `id`) every backoff tick for up to [§16.1: 72 h] while it waits. Acking here would reopen the **existence oracle** §19.3.2 closes — a prober learns the identity exists, and with a plain `KeyTag` (§2.2a) which node holds it, because a non-recipient drops silently at step 4 while a real recipient would ack. The `stored`/`deferred` ack asymmetry is load-bearing for §6.4 and §6.6, not an implementation detail (§19.3.2). |
...
| `ACKED` | (any further `check_duplicate` for the same `id`, from a redelivered copy) | `ACKED` | Idempotent — see `ADDR_OK`'s `duplicate_of_acked` row; a redelivered copy of an already-`ACKED` `id` is a fresh machine instance that dedup-shortcuts straight to `ACKED`. A redelivered copy of a **`DEFERRED`** `id` takes the `duplicate_of_deferred` row instead and is never acked. |
